...
beforeEach(async () => {
  await repository.saveDomain({name: 'bar.com',
    dkim_selector: 'dkim',
...
test('create user', async () => {
  const response = await request(actions.default, {
    locals: { email: 'foo@iroco.co' },
    body: createForm({ user_email: 'qux@bar.com', user_password: 'password', user_role: 2,
      user_password_code: 'code', user_first_name: 'Qux', user_last_name: 'Bar' })
  })
