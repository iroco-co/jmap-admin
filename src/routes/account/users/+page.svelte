<script lang="ts">
  ...
  let roleItems = Object.keys(Role).filter(v => (isNaN(Number(v)) && !v.startsWith("Super"))).map(label => ({label, value: Role[label]}))
  const columns = [
    {
      key: "email",
      title: "email",
...
<section class="account__users">
  ...
  <SvelteTable classNameTable="account__users__table"
               classNameThead="table-primary"
               classNameCell={["account__users__table__cell"]}
               columns="{columns}" rows="{rows}"></SvelteTable>
  ...
  <Modal bind:showModal>
    ...
    <div class="iroco-ui-input">
      ...
      <Select items={roleItems} bind:role value="User"
              id="select_role" placeholder={$_('account.users.modal.select_placeholder')}
